...
pub struct ModelPushConstants {
    ...
    pub active_layer_ids_packed: [u32; 8],
    pub layer_count: u32,
}
...
impl ModelPushConstants {
    /// Pack up to 32 layer IDs (each clamped to u8 range) into 8 u32 words.
    pub fn pack_layer_ids(ids: &[u32], count: u8) -> [u32; 8] {
        let mut packed = [0u32; 8];
        for i in 0..(count as usize).min(32) {
            packed[i / 4] |= (ids[i].min(255) & 0xFF) << ((i % 4) * 8);
        }
        packed
    }
    ...
    pub fn return_renderable(&self) -> Vec<u8> {
        unsafe {
            ...
            let layer_ids: [u8; 32] = transmute(self.active_layer_ids_packed);
