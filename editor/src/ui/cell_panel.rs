...
pub fn cell_search(world: &mut World) -> Result<()> {
    ...
    let mut selected_worldspace: Option<String> = world
        .get_resource::<CellSearchState>()?
        .selected_worldspace
        .clone();
    ...
    let (state, restore) = match world.get_resource::<WindowLayout>() {
        Ok(l) => (l.cell_search.clone(), l.restore > 0),
        Err(_) => return Ok(()),
    };
    ...
    let window = window.show(&ctx, |ui| {
        ...
        ui.horizontal(|ui| {
            ui.horizontal(|ui| {
                ...
                // LEFT: scenes list
                let left_rect = Rect::from_min_size(ui.cursor().min, Vec2::new(panel_w, panel_h));
                                ...
                                .show(ui, |ui| {
                                    ...
                                    for (idx, ws_name) in ws_names.iter().enumerate() {
                                        ...
                                        let is_interior: bool = world
                                            ...
                                            .and_then(|am| am.get_loader::<WorldspaceLoader>())
                                            .and_then(|l| {
                                                l.registry.read().worldspaces.get(ws_name).cloned()
                                            })
                                            .and_then(|v| {
                ...
                let right_rect = Rect::from_min_size(ui.cursor().min, Vec2::new(panel_w, panel_h));
                ...
                egui::Frame::new()
                    ...
                    .show(&mut right, |ui| {
                        ...
                        // parse filter
                        let filter_splits = pending_entity_filter.split(':').collect::<Vec<&str>>();
                        ...
                        if ui.input(|i| i.key_pressed(egui::Key::F2))
                            ...
                            && let Some(entry) = entity_entries.iter().find(|e| e.entity_id == id)
                        {
                            renaming_id = Some(id);
                        ...
                        let tree_active = !show_worldspaces && filter_value.trim().is_empty();

                        let rows: Vec<DisplayRow> = if tree_active {
                            let cell = selected_cell.unwrap_or_else(|| Vector3::new(0, 0, 0));
                            let id_set: HashSet<EntityId> = entity_entries
                                .iter()
                                .filter(|e| e.entity_id.cell == cell)
                                .map(|e| e.entity_id)
                                .collect();
                            let entry_by_id: HashMap<EntityId, &EntityRefEntry> = entity_entries
                                .iter()
                            ...
                            while let Some((id, depth)) = stack.pop() {
                                let has_children =
                                    children_of.get(&id).map(|c| !c.is_empty()).unwrap_or(false);
                        ...
                        ScrollArea::vertical()
                            ...
                            .show(ui, |ui| {
                                ...
                                for (idx, row) in rows.iter().enumerate() {
                                    ...
                                    let arrow_w = 12.0;
                                    let is_sel = pending_selected_entity == Some(entry.entity_id);
                                    let is_clicked =
                                    ...
                                    row_resp.context_menu(|ui| {
                                        ...
                                        if ui.button("Copy Entity").clicked() {
                                            entity_to_copy = world.capture_entity(entry.entity_id);
                                            ui.close();
                                        }
                                        ...
                                        if ui.button("Paste Entity").clicked() {
                                            let s =
                                                world.get_resource::<CellSearchState>().unwrap();
