...
- **Component**: data + behaviour, derive `#[derive(Component)]`. The macro enforces `Clone + Send + Sync + 'static`; you must also `impl Default` and `impl Debug`. Registered globally via `inventory`. Serialization is opt-in: add `#[component(serde)]` plus `#[derive(serde::Serialize, serde::Deserialize)]` and the registry serializes/deserializes it automatically (scene persistence, Lua/hail, asset loaders). Runtime-only fields get `#[serde(skip)]`; `cgmath` vector fields use `#[serde(with = "vec3_seq")]` from `ecs::components::serde_support` to keep the `[x, y, z]` yaml format. Optional derive args: `#[component(serde, skip_if = "method")]` (skip persisting when the predicate is true), `#[component(serde, post_deserialize = "method")]` (hook after every load/apply). Applies are partial updates — fields missing from the yaml keep their current values.
